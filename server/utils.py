...
def row_normalize(mat):
    D = np.asarray(mat.sum(axis=1).flatten())
    D = np.divide(1, D, out=np.zeros_like(D), where=D != 0)
    D = diags(np.asarray(D)[0,:])
    return D @ mat
...
def filter_similarity_matrix(sim, sigma):
    ...
    print('threshold for filtering: {}'.format(threshold))
...
def new_power_method(G, c=0.85, lambda_=1.0, maxiter=100, tol=1e-3, personalization=None):
    """
    r = Wr + (1-c)/(1+alpha) e
    :param G: Networkx DiGraph created by transition matrix W
    :param c: damping factor
...
def reverse_power_method(G, c=0.85, maxiter=100, tol=1e-3, personalization=None):
    # this is calculating r' = alpha * r' * A + (1 - alpha) * vector'
    """
    r = cr'W + (1-c)e
    :param G: Networkx DiGraph created by transition matrix W
    :param c: damping factor
